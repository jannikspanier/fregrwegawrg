...
SETUP_SCRIPT="$(mktemp "/tmp/apexium-fivem-setup.XXXXXX.sh")"
...
tar -xJf /tmp/fx.tar.xz \
  -C /opt/gameserver

curl -fsSL https://github.com/citizenfx/cfx-server-data/archive/refs/heads/master.tar.gz | tar -xz -C /tmp
cp -a /tmp/cfx-server-data-master/resources /opt/gameserver/resources

chmod 0755 /opt/gameserver/run.sh
...
exec ./run.sh +exec data/apexium.cfg +exec data/server.cfg <&3
EOF
