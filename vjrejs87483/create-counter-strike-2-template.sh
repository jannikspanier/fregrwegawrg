...
VMID=9108
TEMPLATE_NAME="counter-strike-2"
STORAGE="local"
BRIDGE="vmbr3"
...
SETUP_SCRIPT="$(mktemp "/tmp/apexium-counter-strike-2-setup.XXXXXX.sh")"
...
export LD_LIBRARY_PATH=/opt/gameserver/.steam/sdk64:/opt/gameserver/game/bin/linuxsteamrt64:/opt/gameserver/game/csgo/bin/linuxsteamrt64
A=(-dedicated -usercon -port "$GAME_PORT" +map "${CS2_MAP:-de_dust2}" +hostname "$SERVER_NAME")
if [ -n "${STEAM_GSLT:-}" ]; then A+=(+sv_setsteamaccount "$STEAM_GSLT"); fi
exec ./game/bin/linuxsteamrt64/cs2 "${A[@]}" <&3
EOF
...
if [ "$VERSION" = "latest" ]; then
    :
    runuser -u gameserver -- env HOME="$TMP/home" "$TMP/steamcmd/steamcmd.sh" "${APP_ARGS[@]}" +force_install_dir "$ROOT" +login anonymous +app_update 730 validate +quit
    ...
    runuser -u gameserver -- env HOME="$TMP/home" "$TMP/steamcmd/steamcmd.sh" "${APP_ARGS[@]}" +force_install_dir "$ROOT" +login anonymous +app_update 730 -beta "$BRANCH" validate +quit
    ...
    runuser -u gameserver -- env HOME="$TMP/home" "$TMP/steamcmd/steamcmd.sh" "${APP_ARGS[@]}" +login anonymous +download_depot 730 "$DEPOT" "$MANIFEST" +quit
    CONTENT="$TMP/home/Steam/steamapps/content/app_730/depot_$DEPOT"; [ -d "$CONTENT" ] || { echo "Steam-Manifest-Inhalt fehlt" >&2; exit 2; }; cp -a "$CONTENT/." "$ROOT/"
