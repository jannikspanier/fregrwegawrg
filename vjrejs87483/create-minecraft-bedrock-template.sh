...
VMID=9101
TEMPLATE_NAME="minecraft-bedrock"
STORAGE="local"
BRIDGE="vmbr3"
BASE_TEMPLATE="${STORAGE}:vztmpl/apexium-ubuntu-24.04-minbase.tar.zst"
...
SETUP_SCRIPT="$(mktemp "/tmp/apexium-minecraft-bedrock-setup.XXXXXX.sh")"
...
curl -4 -fL -A 'Mozilla/5.0' -o /tmp/bedrock-server.zip "$BEDROCK_URL"
unzip -o /tmp/bedrock-server.zip -d /opt/gameserver
chmod 0755 /opt/gameserver/bedrock_server
...
#!/usr/bin/env bash
cd /opt/gameserver
sed -i "s/^server-port=.*/server-port=$GAME_PORT/;s/^max-players=.*/max-players=$MAX_PLAYERS/;s/^server-name=.*/server-name=$SERVER_NAME/;s/^allow-list=.*/allow-list=false/" server.properties
export LD_LIBRARY_PATH=/opt/gameserver
