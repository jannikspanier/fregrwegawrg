...
SETUP_SCRIPT="$(mktemp "/tmp/apexium-palworld-setup.XXXXXX.sh")"
...
#!/usr/bin/env bash
exec /usr/local/lib/apexium/source_rcon.pl RCON_PORT PALWORLD_ADMIN_PASSWORD "$@"
EOF
...
#!/usr/bin/env bash
cd /opt/gameserver
CFG=/opt/gameserver/Pal/Saved/Config/LinuxServer/PalWorldSettings.ini
mkdir -p "$(dirname "$CFG")"
if [ ! -f "$CFG" ]; then cp /opt/gameserver/DefaultPalWorldSettings.ini "$CFG"; fi
perl -0pi -e '
  s/RCONEnabled=(?:True|False)/RCONEnabled=True/g;
  s/RCONPort=\d+/"RCONPort=".$ENV{RCON_PORT}/ge;
  s/AdminPassword="[^"]*"/"AdminPassword=\"".$ENV{PALWORLD_ADMIN_PASSWORD}."\""/ge;
  s/ServerName="[^"]*"/"ServerName=\"".$ENV{SERVER_NAME}."\""/ge;
  s/ServerPlayerMaxNum=\d+/"ServerPlayerMaxNum=".$ENV{MAX_PLAYERS}/ge;
' "$CFG"
exec ./PalServer.sh -port="$GAME_PORT" -queryport="$QUERY_PORT" -players="$MAX_PLAYERS"
EOF
...
if [ "$VERSION" = "latest" ]; then
    :
    runuser -u gameserver -- env HOME="$TMP/home" "$TMP/steamcmd/steamcmd.sh" "${APP_ARGS[@]}" +force_install_dir "$ROOT" +login anonymous +app_update 2394010 validate +quit
    ...
    runuser -u gameserver -- env HOME="$TMP/home" "$TMP/steamcmd/steamcmd.sh" "${APP_ARGS[@]}" +force_install_dir "$ROOT" +login anonymous +app_update 2394010 -beta "$BRANCH" validate +quit
    ...
    runuser -u gameserver -- env HOME="$TMP/home" "$TMP/steamcmd/steamcmd.sh" "${APP_ARGS[@]}" +login anonymous +download_depot 2394010 "$DEPOT" "$MANIFEST" +quit
    CONTENT="$TMP/home/Steam/steamapps/content/app_2394010/depot_$DEPOT"; [ -d "$CONTENT" ] || { echo "Steam-Manifest-Inhalt fehlt" >&2; exit 2; }; cp -a "$CONTENT/." "$ROOT/"
