...
test -x /opt/gameserver/srcds_run_x64 || {
  echo "SteamCMD hat den Garry's-Mod-x86-64-Server nicht vollstaendig installiert: /opt/gameserver/srcds_run_x64 fehlt." >&2
  exit 1
}
...
mkdir -p "$(dirname "$LOG")"
: > "$LOG"
# -conclearlog entfernt console.log waehrend tail sie beobachtet. Da wir die Datei
# hier ohnehin leeren, reicht -condebug und es gibt keinen tail-Reopen-Fehler.
tail -n0 -F "$LOG" 2>/dev/null &
A=(-game garrysmod -console -condebug -port "$GAME_PORT" +maxplayers "$MAX_PLAYERS" +map "${GMOD_MAP:-gm_construct}" +hostname "$SERVER_NAME")
if [ -n "${STEAM_GSLT:-}" ]; then A+=(+sv_setsteamaccount "$STEAM_GSLT"); fi
exec ./srcds_run_x64 "${A[@]}" <&3 >/dev/null 2>&1
EOF
...
if [ "$VERSION" = "latest" ]; then
    APP_ARGS+=( -beta x86-64 )
    runuser -u gameserver -- env HOME="$TMP/home" "$TMP/steamcmd/steamcmd.sh" "${APP_ARGS[@]}" +force_install_dir "$ROOT" +login anonymous +app_update 4020 validate +quit
    ...
    runuser -u gameserver -- env HOME="$TMP/home" "$TMP/steamcmd/steamcmd.sh" "${APP_ARGS[@]}" +force_install_dir "$ROOT" +login anonymous +app_update 4020 -beta "$BRANCH" validate +quit
    ...
    runuser -u gameserver -- env HOME="$TMP/home" "$TMP/steamcmd/steamcmd.sh" "${APP_ARGS[@]}" +login anonymous +download_depot 4020 "$DEPOT" "$MANIFEST" +quit
    CONTENT="$TMP/home/Steam/steamapps/content/app_4020/depot_$DEPOT"; [ -d "$CONTENT" ] || { echo "Steam-Manifest-Inhalt fehlt" >&2; exit 2; }; cp -a "$CONTENT/." "$ROOT/"
else echo "Ungültige Steam-Version: $VERSION" >&2; exit 2; fi
mkdir -p "$ROOT/.steam/sdk64"; cp "$TMP/steamcmd/linux64/steamclient.so" "$ROOT/.steam/sdk64/steamclient.so"; test -x "$ROOT/srcds_run_x64" || { echo "Garrys-Mod-Binary fehlt" >&2; exit 2; }
printf '%s
